...
def f03(*words):
    truth = " ".join(words)  # This is broken (now fixed :)).
...
def f06(string1, string2):
    if(len(string1) > len(string2)):
        ...
        print("{} has only {}% the number of chars of {}".format(string2,len(string2)*100/len(string1),string1))
    elif(len(string2) > len(string1)):
        diff = len(string2)-len(string1)
        print("{} is longer than {} by {} chars".format(string2,string1,diff))
        print("{} has only {}% the number of chars of {}".format(string1,len(string1)*100/len(string2),string2))
...
###############################################################################
def f09():
    numbers = [x for x in range(1,500) if x % 3 == 0 or x % 5 == 0 ]
    sum = 0
...
def f12():
    with open("log_file.txt","w") as output_file:
        user_input = ""
        convert = 0.0
        list_of_converted_inputs = []
        while(not user_input == "done"):
            user_input = input("> ")
            try:
                convert = float(user_input)
                list_of_converted_inputs.append(convert)
            except:
                output_file.write("{}\n".format(user_input))
        print(list_of_converted_inputs)
...
def f16(list1, list2):
    ...
    for idx in range(len(list1)):
        tuple_list.append((list1[idx],list2[idx]))
    print(tuple_list)
...
def f17():
    list_of_words = []
    with open("few_words.txt","r") as file_object:
        file_object.readline()
        iterator = 0
        for each_word in file_object.readline().split():
            list_of_words.append((iterator,each_word))
            iterator += 1
        print(list_of_words)
    ...
    list_ = list_of_words  # Change to your list variable name
...
def f19(d):
    for each_key in d.keys():
        if(d[each_key] + len(each_key) % 2 == 0):
            d[each_key] += len(each_key)
        else:
            d[each_key] = ord(each_key[-1])
    print(d)
...
def f21(word):
    for each_letter in list(word):
        if(not ord(each_letter) in range(ord('A'),ord('Z')+1)):
            return False
    return True
...
def f22():
    ...
    with open("few_words.txt","r") as file_object:
        for each_word in file_object.read().split():
            if(f21(each_word)):
                list_of_capitalized_words.append(each_word)
        print(list_of_capitalized_words)
        list_of_capitalized_words = list(set(list_of_capitalized_words))
        if("Python" in list_of_capitalized_words):
            list_of_capitalized_words.remove("Python")
            list_of_capitalized_words = ["Python"]+list_of_capitalized_words
            list_of_capitalized_words[0].upper()
            if("Magic" in list_of_capitalized_words):
                list_of_capitalized_words.remove("Magic")
            list_of_capitalized_words[0] += "!"
            list_of_capitalized_words = list_of_capitalized_words.remove("PYTHON!")
            list_of_capitalized_words = ["PYTHON!"]+[list_of_capitalized_words]
            print(list_of_capitalized_words)
...
def f23(_list):
    for idx in range(0,3):
        if(_list[idx][0] == _list[idx][1] and _list[idx][0] == _list[idx][2]):
            if(_list[idx][0] == "x"):
                print("x, row{}".format(idx))
            else:
                print("o, row{}".format(idx))
        if(_list[0][idx] == _list[1][idx] and _list[0][idx] == _list[2][idx]):
            if(_list[0][idx] == "x"):
                print("x, col{}".format(idx))
            else:
                print("o, col{}".format(idx))
        if(_list[0][0] == _list[1][1] and _list[0][0] == _list[2][2]):
            if(_list[0][0] == "x"):
                print("x, falling_front_diag")
            else:
                print("o, falling_front_diag")
        if(_list[0][2] == _list[1][1] and _list[0][2] == _list[2][0]):
            if(_list[0][2] == "x"):
                print("x, falling_back_diag")
            else:
                print("o, falling_back_diag")
